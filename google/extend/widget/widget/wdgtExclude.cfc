	...
	public string function process( required string content, required struct args ) {
		...
		try {
			...
			if(local.theUrl.searchBoolean('exclude')) {
				return 'Marking browser as <strong>excluded</strong>. <script>jQuery(function() { jQuery.track(''_setCustomVar'', 5, ''_exclude'', true, 1); });</script>';
			}
			
			// Check for inclusion flag
			if(local.theUrl.searchBoolean('include')) {
				return 'Marking browser as <strong>included</strong>. <script>jQuery(function() { jQuery.track(''_setCustomVar'', 5, ''_exclude'', false, 1); });</script>';
			}
