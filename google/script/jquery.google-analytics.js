...
(function($) {
	 ...
	 * 
	 * Usage: <script type="text/javascript"> $.trackPage('UA-xxx-xxx',
	 * options); </script>
	 * 
	 * Parameters: account_id - Your Google Analytics account ID. options - An
	 * object containing one or more optional parameters: - onload - boolean -
	 * If false, the Google Analytics code is loaded when this method is called
	 * instead of on window.onload. - status_code - The HTTP status code of the
	 * current server response. If this is set to something other than 200 then
	 ...
	 * callback - function to be executed after the Google Analytics code is
	 * laoded and initialized
	 * 
	 */
			...
			if (typeof _gat !== undefined) {
				...
				if (settings.status_code === null || settings.status_code === 200) {
					...
					pageTracker._trackPageview("/" + settings.status_code +
							".html?page=" + document.location.pathname +
	 ...
	 * 
	 * category - required string used to group events action - required string
	 * used to define event type, eg. click, download label - optional label to
	 * attach to event, eg. buy value - optional numerical value to attach to
	 * event, eg. price skip_internal - optional boolean value. If true then
	 * internal links are not tracked.
	 * 
	 */
